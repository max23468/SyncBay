...
test("bundle budget accepts small files and reports exceeded limits", () => {
  ...
  assert.deepEqual(measureBundleBudget(root, { clientTotal: 1, route: 1, css: 1, server: 1 }).exceeded.map(([key]) => key).sort(), ["clientTotal", "css", "route", "server"]);
  const accepted = spawnSync(process.execPath, ["scripts/syncbay-bundle-budget.mjs", `--root=${root}`]);
  assert.equal(accepted.status, 0);
  fs.writeFileSync(path.join(root, "build/client/assets/app.catalog.js"), randomBytes(250 * 1024));
  const rejected = spawnSync(process.execPath, ["scripts/syncbay-bundle-budget.mjs", `--root=${root}`]);
