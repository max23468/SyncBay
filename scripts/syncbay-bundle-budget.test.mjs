...
test("bundle budget fails when build artifacts are missing", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "syncbay-bundle-missing-"),
  );
...
test("bundle budget accepts small files and reports exceeded limits", () => {
  ...
  fs.writeFileSync(
    path.join(root, "build/client/assets/app.catalog.js"),
    "small",
  );
  fs.writeFileSync(
    path.join(root, "build/client/assets/syncbay-embedded.css"),
    "small",
  );
