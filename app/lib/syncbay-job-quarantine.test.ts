...
test("treats failed jobs with remaining attempts as retrying", () => {
  ...
  assert.equal(isSyncJobQuarantined({ attempts: 2, maxAttempts: 5, status: "FAILED" }), false);
});
...
test("never quarantines settled or in-flight jobs", () => {
  ...
  for (const status of ["PENDING", "RETRYING", "RUNNING"]) {
    assert.equal(classifySyncJobQuarantine({ attempts: 9, maxAttempts: 5, status }), "retrying");
  }
});

test("normalizes invalid attempt bounds", () => {
  // maxAttempts 0 is invalid and normalizes to 1, so 0 attempts is not exhausted.
  assert.equal(hasExhaustedSyncJobAttempts({ attempts: 0, maxAttempts: 0 }), false);
  assert.equal(hasExhaustedSyncJobAttempts({ attempts: 1, maxAttempts: 0 }), true);
  assert.equal(hasExhaustedSyncJobAttempts({ attempts: Number.NaN, maxAttempts: 3 }), false);
});
