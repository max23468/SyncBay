...
test("keeps stock, price, product status and SKU conflicts manual-only", () => {
  for (const field of ["quantity", "price", "status", "sku"]) {
    ...
    assert.equal(getConflictResolutionSafety(field, "KEEP_SHOPIFY").mode, "manual_only");
    assert.equal(getConflictResolutionSafety(field, "REALIGN_FROM_EBAY").mode, "manual_only");
  }
});
...
test("allows only keep-shopify description conflicts as batch-safe", () => {
  ...
  assert.deepEqual(getSafeBatchConflictResolutions("description"), ["KEEP_SHOPIFY"]);
  assert.equal(getConflictResolutionSafety("description", "KEEP_SHOPIFY").mode, "batch_safe");
  assert.equal(getConflictResolutionSafety("description", "REALIGN_FROM_EBAY").mode, "guarded");
  assert.equal(getConflictResolutionSafety("description", "IGNORE_FIELD").mode, "manual_only");
});
...
test("treats title and image conflict actions as guarded decisions", () => {
  for (const field of ["title", "images"]) {
    ...
    assert.deepEqual(getSafeBatchConflictResolutions(field), []);
    assert.equal(getConflictResolutionSafety(field, "KEEP_SHOPIFY").mode, "guarded");
    assert.equal(getConflictResolutionSafety(field, "REALIGN_FROM_EBAY").mode, "guarded");
  }
});
...
test("treats unknown conflict fields as manual-only", () => {
  ...
  assert.match(getConflictResolutionSafety("custom_field", "KEEP_SHOPIFY").detail, /manuale/i);
});

test("treats only missing conflicts as stale batch resolution errors", () => {
  assert.equal(isStaleConflictResolutionError(new Response("missing", { status: 404 })), true);
  assert.equal(isStaleConflictResolutionError(new Response("failed", { status: 500 })), false);
