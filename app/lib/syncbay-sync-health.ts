export type CatalogSyncHealthStatus = "disabled" | "due" | "fresh" | "overdue" | "running";
...
export function getCatalogSyncHealth(input: {
  ...
  const nextDueAt = input.latestIncrementalFinishedAt
    ? new Date(input.latestIncrementalFinishedAt.getTime() + input.syncTargetSeconds * 1000)
    : input.now;
  const secondsUntilDue = Math.round((nextDueAt.getTime() - input.now.getTime()) / 1000);
  const overdueGraceSeconds = normalizeOverdueGraceSeconds(input.overdueGraceSeconds);
  const overdueAt = new Date(nextDueAt.getTime() + (overdueGraceSeconds + 1) * 1000);
  ...
  return {
    ...
    secondsUntilDue,
    status:
      secondsUntilDue > 0 ? "fresh" : secondsUntilDue < -overdueGraceSeconds ? "overdue" : "due",
  };
}
