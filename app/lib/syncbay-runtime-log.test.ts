...
test("uses the Vercel request id without reading request payloads", () => {
  assert.equal(
    getSyncBayRequestId(
      new Request("https://example.test", {
        headers: { "x-vercel-id": "fra1::safe-id" },
      }),
    ),
    "fra1::safe-id",
  );
  assert.match(
    getSyncBayRequestId(
      new Request("https://example.test", {
        headers: { "x-vercel-id": "unsafe value" },
      }),
    ),
    /^[0-9a-f-]{36}$/u,
  );
...
test("production samples healthy events but always keeps warnings and slow events", () => {
  ...
  try {
    ...
    assert.equal(
      shouldLogSyncBayRuntimeEvent(base, () => 0.9),
      false,
    );
    assert.equal(
      shouldLogSyncBayRuntimeEvent({ ...base, durationMs: 1_001 }, () => 0.9),
      true,
    );
    assert.equal(
      shouldLogSyncBayRuntimeEvent({ ...base, level: "warn" }, () => 0.9),
      true,
    );
  } finally {
