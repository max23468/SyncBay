...
export async function requestEbayRestJson<T = unknown>(input: {
  ...
  try {
    ...
    response = await fetch(input.url, {
      headers,
      signal: input.signal ?? AbortSignal.timeout(DEFAULT_EBAY_REST_TIMEOUT_MS),
    });
  } catch (cause) {
    throw new Error(`${input.operation}: errore di rete.`, { cause });
  }
  ...
  try {
    ...
    throw new Error(`${input.operation}: risposta JSON non valida (HTTP ${response.status}).`, {
      cause,
    });
  }
