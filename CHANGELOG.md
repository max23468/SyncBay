...
- Ordinati import e sincronizzazioni eBay -> Shopify dalla pubblicazione eBay
  più vecchia alla più recente, mantenendo l'ordine anche tra batch e durante
  la creazione dei prodotti Shopify.
...
- Vincolata la connessione per lo staging delle immagini all'indirizzo pubblico
  già validato, impedendo che una seconda risoluzione DNS raggiunga reti private.
...
- Il runner limita al primo slot incrementale la priorità dei delta live e torna
  poi all'ordine FIFO, evitando che un flusso continuo di eventi eBay affami i
  job di riconciliazione catalogo già aperti.
...
- Il rilevamento dei conflitti legge la baseline dopo la risposta Shopify, evitando conflitti transitori quando un job SyncBay concorrente aggiorna la baseline durante la lettura del provider.
...
- Database Supabase: recuperato spazio dalla tabella vuota delle richieste
  account deletion e ridotta la crescita futura con audit webhook Shopify
  compatto, retention a 30 giorni per audit webhook e 45 giorni per job
  riusciti.
...
- Ordini Shopify: la pulizia automatica dei conflitti quantità dopo una write
  stock eBay è best-effort e non fa fallire un aggiornamento eBay già
  completato se Shopify Admin non risponde.
...
- Doctor collezioni: anche la lettura di copertura e proposte passa dal modello
  Shopify Admin 2026-07 `collection sources / conditions` invece del campo
  deprecato `Collection.ruleSet`, con conversione pura e testata delle condizioni
  tipizzate nella forma regole usata dal motore proposte.
...
- Panoramica: l'avviso di allineamento eBay non scatta più per ritardi
  transitori entro il normale margine del cron, evita il doppio hero per lo
  stesso ritardo e distingue i fallimenti recenti da un sync solo in attesa del
  prossimo controllo.
...
- Catalogo esistente: le categorie Shopify incerte non bloccano più righe con
  match automatico forte; SyncBay salta solo l'allineamento categoria e mantiene
  applicabile il collegamento sicuro al prodotto Shopify esistente.
...
- Catalogo esistente: la lookup mirata degli SKU Shopify fuori dalla finestra
  iniziale usa la query prodotti già collaudata dalla scansione principale,
  evitando che i match forti restino bloccati tra i casi da rivedere.

## [1.0.12] — 2026-07-04

### Correzioni

- Catalogo esistente: la preview cerca su Shopify anche varianti con SKU mirati
  dagli ItemID/SKU eBay letti, recuperando i prodotti fuori dalla finestra
  iniziale di matching senza aumentare il consumo della Trading API.
...
- Catalogo esistente: il matching considera lo SKU Shopify uguale all'ItemID
  eBay come segnale forte, evitando revisioni manuali inutili sui cataloghi
  importati da app legacy che salvano l'ItemID nello SKU Shopify.
...
- OAuth eBay: i link `Collega eBay` includono il dominio shop nel contesto
  embedded, evitando il `401` Shopify quando App Bridge apre
  `/auth/ebay/start.data` senza shop.
...
- Impostazioni: la lista dei canali Shopify mostra il nome leggibile della
  publication e non i titoli tecnici `Channel Catalog ...` restituiti dai
  cataloghi Shopify.
...
- Runtime: la detection dei conflitti Shopify confronta prezzo e quantità sulla
  variante mappata, evitando falsi conflitti quando Shopify restituisce prima
  una variante diversa del prodotto.
...
- Runtime: la configurazione Prisma/pg applica `uselibpqcompat=true` quando la
  URL Supabase usa `sslmode=require`, evitando errori TLS
  `self-signed certificate in certificate chain` sul pooler Supabase in Vercel.

## [0.50.4] — 2026-07-02

### Sotto il cofano

- Dipendenze: aggiornate le patch sicure di React Router 7, Vite, TypeScript
  ESLint, Prettier e `@eslint-react/eslint-plugin`; aggiornato React Doctor a
  `0.6.0`.
...
- Importazione: il takeover `reuseOnly` preserva la variante Shopify salvata nel
  mapping e non ricade sulla prima variante del prodotto durante riallineamento,
  stock, prezzi e snapshot.
...
- Egress Supabase: la baseline dei conflitti Shopify legge solo le colonne
  necessarie da `ProductSnapshot` e filtra in SQL le descrizioni valide,
  evitando scan ripetuti di `payload` durante le raffiche webhook.
...
- Monitor egress: il classificatore delle CTE distingue le query read-only dalle
  mutation `WITH ... INSERT ... SELECT`, evitando di ricontare righe DML come
  proxy di egress.
...
- Loader embedded: estesa la strumentazione server-side redatta anche a
  Panoramica, Importazione e Attività per confrontare tutte le pagine principali.
...
- Catalogo e conflitti: le letture display non caricano più
  `ProductSnapshot.payload` per risolvere le miniature, usando la colonna
  dedicata `ProductMapping.thumbnailUrl` e il fallback cache Shopify.
...
- Catalogo e conflitti: le miniature leggono solo URL sintetici dagli snapshot
  prodotto, evitando di trasferire payload JSON storici a ogni apertura pagina.
