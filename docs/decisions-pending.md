...
| Lettura listing eBay live | Chiusa per preview pilota | Preview live collegata a Inventory API per inventory item con offer pubblicate e fallback Trading API `GetMyeBaySelling` per listing attivi storici/Seller Hub | Prima dell'import completo serve verificare sullo shop pilota che la lettura combinata restituisca dati reali corretti e importabili. |
