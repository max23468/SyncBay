...
- `/api/jobs/run-due` è il runner HTTP protetto da `CRON_SECRET` per riprendere job `IMPORT_CATALOG` dovuti. La schedule Supabase Cron `syncbay-run-due-jobs` è attiva ogni 5 minuti e legge il secret da Supabase Vault, senza valore segreto in repo o documentazione. Il runner pianifica i sync incrementali solo quando il prossimo `runAfter` è già scaduto, così non anticipa l'intervallo scelto dal negoziante né i `runAfter` usati come backoff provider. I retry reali recuperano i listing per `ItemID` via Trading API `GetItem` e chiudono il job originale senza lasciarlo `RUNNING`.
...
- `/api/diagnostics/database` è l'endpoint diagnostico interno per verificare la
  ...
  presenza dei parametri logici `connection_limit`, `pool_timeout` e
  `uselibpqcompat` da cui Prisma 7/`pg` ricavano pool e semantica TLS runtime,
  regione Vercel runtime e presenza delle URL attese; non espone hostname
  completi, credenziali o stringhe di connessione.
...
- estensione `pg_cron`;
- estensione `pg_net`;
- schedule `syncbay-run-due-jobs` ogni 5 minuti per riprendere job `IMPORT_CATALOG` dovuti;
