# Archivio roadmap - SyncBay

Archivio storico della vecchia roadmap di fondazione. Non è la roadmap
canonica: priorità e prossimi passi correnti stanno in [`ROADMAP.md`](./ROADMAP.md).

Snapshot storico della roadmap precedente.
...
| Fatto | Modello mapping/snapshot/conflitti          | Schema Prisma e migration applicati su Supabase; l'import pilota registra mapping, snapshot, job e audit per prodotti Shopify creati o riusati      |
...
| Stato    | Voce                                  | Note                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fatto    | Connessione Shopify custom app        | Precedente ambiente pilota verificato via Shopify CLI preview, sessione persistita e audit installazione registrato                                                                                                                                                                                                                                                                                                                   |
...
| Fatto    | Onboarding guidato                    | Readiness dashboard, area Impostazioni embedded e wizard import preview predisposti; scelta location Shopify salvabile, default prodotti `Pubblicato` con override `Bozza`, preview live Inventory API con fallback Trading API/GetItem sui primi 10 listing del batch preview, SKU fallback e regole dry-run MVP codificate                                                                                                             |
| In corso | Import iniziale fino a 2.000 prodotti | Batch 50 verificato con import Shopify idempotente; l'ultimo job reale ha gestito 50 listing con 26 nuovi prodotti Shopify e 24 riusi senza duplicati. L'import controllato registra mapping con product/variant GID, snapshot, job e audit, usa `Pubblicato` come default per i nuovi import, consente `Bozza` da Impostazioni, allinea tracking scorte/quantità Shopify sulla location predefinita e copia tutte le immagini eBay disponibili con fallback Supabase Storage temporaneo. Resta espandere copertura fino al limite MVP |
...
1. Disegnare l'import completo fino a 2.000 prodotti mantenendo idempotenza, mapping e snapshot.
2. Introdurre orchestrazione batch/paginazione oltre i 50 listing per run controllata.
